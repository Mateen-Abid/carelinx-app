...
const ClinicAdminSidebar: React.FC<ClinicAdminSidebarProps> = ({ isDarkMode = false, onDarkModeToggle }) => {
    ...
    <aside className={`${isCollapsed ? 'w-16' : 'w-64'} bg-[#0C2243] h-screen flex flex-col transition-all duration-300 sticky top-0 self-start`}>
      ...
      <div className="p-4 border-t border-white/10 space-y-1">
        <button
          ...
          className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'justify-start'} gap-3 px-3 py-2.5 rounded-lg transition-colors ${
            location.pathname === '/clinic-admin/settings'
              ? 'bg-[#00FFA2] text-[#0C2243]'
              : 'text-white/80 hover:bg-white/10 hover:text-white'
          }`}
...
export default ClinicAdminSidebar;
