...
const AdminSidebar: React.FC<AdminSidebarProps> = ({ isDarkMode = false, onDarkModeToggle }) => {
  ...
  const navItems = [
    ...
    { path: '/admin/appointments', label: t('Appointments'), icon: Calendar },
    { path: '/admin/doctors', label: t('Doctors'), icon: Stethoscope },
    { path: '/admin/services', label: t('Services'), icon: ClipboardList },
    { path: '/admin/patients', label: t('Patients'), icon: Users },
  ];

  const isActive = (path: string) => location.pathname === path;

  return (
    <aside className={`${isCollapsed ? 'w-16' : 'w-64'} bg-[#0C2243] h-screen flex flex-col transition-all duration-300 sticky top-0 self-start`}>
      {/* Top border line */}
      <div className="absolute top-0 left-0 right-0 h-0.5 bg-[#00FFA2]"></div>
      ...
      <div className="p-4 border-b border-white/10 relative">
        <div className="flex items-center justify-between gap-2">
          <button 
            ...
            className={`flex items-center ${isCollapsed ? 'justify-center w-full' : 'justify-start flex-1'} transition-all`}
          >
            <svg width="100%" height="100%" viewBox="0 0 431 115" fill="none" xmlns="http://www.w3.org/2000/svg" className={`${isCollapsed ? 'h-6' : 'h-8'} w-auto`}>
          ...
          {!isCollapsed && (
            <div
              className={`w-11 h-6 rounded-full transition-colors relative ${
                isDarkMode ? 'bg-[#00FFA2]' : 'bg-gray-600'
              }`}
            >
              <div
